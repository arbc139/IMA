...
with db.cursor(pymysql.cursors.DictCursor) as cursor:
  query = 'SELECT * FROM PROSTATE_GENES WHERE SYMBOL REGEXP "\'.*\'" ORDER BY S_ID' if START_S_ID is None \
    else 'SELECT * FROM PROSTATE_GENES WHERE SYMBOL REGEXP "\'.*\'" AND S_ID > %s ORDER BY S_ID' % (START_S_ID)
...
with db.cursor(pymysql.cursors.DictCursor) as cursor:
  cursor.executemany(
    'UPDATE PROSTATE_GENES SET SYMBOL=%s WHERE S_ID=%s',
    values
  )
db.commit()
