...
for gene in all_genes:
  ...
  if max_doc:
    ...
    if not gene_family_info:
      ...
      with db.cursor(pymysql.cursors.DictCursor) as cursor:
        cursor.execute(
          'UPDATE PROSTATE_GENES SET HGNC_ID="%s", SYMBOL="%s", MAX_SCORE=%s, MESH_NAME="%s" WHERE S_ID=%s',
    ...
    print('UPDATE PROSTATE_GENES SET HGNC_ID="%s", SYMBOL="%s", MAX_SCORE=%s, MESH_NAME="%s", HGNC_FAMILY_NAME="%s", NAME_SCORE=%s WHERE S_ID=%s' % (
    ...
    with db.cursor(pymysql.cursors.DictCursor) as cursor:
      cursor.execute(
        'UPDATE PROSTATE_GENES SET HGNC_ID="%s", SYMBOL="%s", MAX_SCORE=%s, MESH_NAME="%s", HGNC_FAMILY_NAME="%s", NAME_SCORE=%s WHERE S_ID=%s',
        (max_doc['hgnc_id'], max_doc['symbol'], max_doc['score'], mesh_name, gene_family_info['GENE_FAMILY_NAME'], name_score, gene['S_ID'])
      )
    db.commit()
    print('Update gene time:', get_elapsed_seconds(get_current_millis(), elapsed_millis))
  ...
  if not gene_family_info:
    ...
    with db.cursor(pymysql.cursors.DictCursor) as cursor:
      cursor.execute(
        'UPDATE PROSTATE_GENES SET MESH_NAME="%s" WHERE S_ID=%s',
        (mesh_name, gene['S_ID'])
      )
    db.commit()
    print('Update gene time:', get_elapsed_seconds(get_current_millis(), elapsed_millis))
  ...
  if not mesh_name:
    ...
    with db.cursor(pymysql.cursors.DictCursor) as cursor:
      cursor.execute(
        'UPDATE PROSTATE_GENES SET HGNC_FAMILY_NAME="%s" WHERE S_ID=%s',
  ...
  print('UPDATE PROSTATE_GENES SET MESH_NAME="%s", HGNC_FAMILY_NAME="%s", NAME_SCORE=%s WHERE S_ID=%s' % (
  ...
  with db.cursor(pymysql.cursors.DictCursor) as cursor:
    cursor.execute(
      'UPDATE PROSTATE_GENES SET MESH_NAME="%s", HGNC_FAMILY_NAME="%s", NAME_SCORE=%s WHERE S_ID=%s',
      (mesh_name, gene_family_info['GENE_FAMILY_NAME'], name_score, gene['S_ID'])
    )
  db.commit()
  print('Update gene time:', get_elapsed_seconds(get_current_millis(), elapsed_millis))
