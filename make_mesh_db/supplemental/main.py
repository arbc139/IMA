...
root = et.parse('../input/supp2017.xml').getroot()
print('Parse xml time:', get_elapsed_seconds(get_current_millis(), elapsed_millis))

elapsed_millis = get_current_millis()
records = root.findall('SupplementalRecord')
print('Find all records time:', get_elapsed_seconds(get_current_millis(), elapsed_millis))

rows = []
for record in records:
  uid = record.findtext('SupplementalRecordUI')
  name = record.find('SupplementalRecordName').findtext('String')
  tree_numbers = []
  for tree_number_record in record.findall('TreeNumberList'):
    tree_numbers.append(tree_number_record.findtext('TreeNumber'))
  
  print('INSERT INTO MESH_SUPPLEMENTAL (UID, NAME, TREE_NUMBERS) VALUES (%s, %s, %s)' % (
    uid, name, str(tree_numbers)))
  
  rows.append([uid, name, str(tree_numbers)])

elapsed_millis = get_current_millis()
# Send a query to insert MESH_SUPPLEMENTAL.
with db.cursor(pymysql.cursors.DictCursor) as cursor:
  cursor.executemany(
    'INSERT INTO MESH_SUPPLEMENTAL (UID, NAME, TREE_NUMBERS) VALUES (%s, %s, %s)',
    rows
  )
db.commit()
